    ...
    >
      ...
      <StatSection title="Overall Population">
        ...
        <StatRow 
          ...
          value={predators.length > 0 ? (prey.length / predators.length).toFixed(2) : "∞"} 
          color="#ffaa00" 
        />
      </StatSection>
      ...
      <StatSection title="Prey Statistics">
        ...
        <StatRow 
          label="Min Energy" 
          value={preyEnergies.length > 0 ? Math.min(...preyEnergies).toFixed(1) : "N/A"} 
          color="#666" 
        />
        <StatRow 
          label="Max Energy" 
          value={preyEnergies.length > 0 ? Math.max(...preyEnergies).toFixed(1) : "N/A"} 
          color="#666" 
        />
      </StatSection>
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: "11px", color: "#aaa" }}>
                <div>Avg Energy: <span style={{ color: "#fff" }}>{avgEnergy.toFixed(1)}</span></div>
                <div>Avg Age: <span style={{ color: "#fff" }}>{avgAge.toFixed(1)}s</span></div>
              </div>
            </div>
          );
        })}
        {Object.keys(preyByType).length === 0 && (
          <div style={{ color: "#666", fontSize: "12px", textAlign: "center", padding: "12px" }}>
            No prey alive
          </div>
        )}
      </StatSection>
      ...
      <StatSection title="Predator Statistics">
        ...
        <StatRow 
          label="Min Energy" 
          value={predatorEnergies.length > 0 ? Math.min(...predatorEnergies).toFixed(1) : "N/A"} 
          color="#666" 
        />
        <StatRow 
          label="Max Energy" 
          value={predatorEnergies.length > 0 ? Math.max(...predatorEnergies).toFixed(1) : "N/A"} 
          color="#666" 
        />
        <StatRow 
      ...
      <StatSection title="Environment">
        <StatRow label="Obstacles" value={state.obstacles.length} color="#ff4444" />
        <StatRow label="Perception Radius" value={state.perceptionRadius} color="#888" />
