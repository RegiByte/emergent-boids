// Health Bar Vertex Shader
// Renders rectangular health bars above boids (only when damaged)
...
// Per-instance attributes (one per health bar)
attribute vec2 boidPos;       // Boid world position
attribute float healthPercent; // Health percentage (0-1)
attribute vec3 barColor;      // Bar color (green/yellow/red based on health)
...
void main() {
  ...
  if (layerType > 0.5 && layerType < 1.5) {
    // Fill bar: scale width by health percentage
    scaledPos.x *= healthPercent;
  }
