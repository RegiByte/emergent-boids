/**
 * Shared Memory Utilities for Parallel Simulation
 *
...
export function createSharedBoidViews(
  ...
  return {
    ...
    energy0: new Float32Array(buffer, layout.energy0Offset, layout.boidCount),
    energy1: new Float32Array(buffer, layout.energy1Offset, layout.boidCount),
    health0: new Float32Array(buffer, layout.health0Offset, layout.boidCount),
    health1: new Float32Array(buffer, layout.health1Offset, layout.boidCount),
 ...
 */
export function getActiveStanceEnteredAtFrame(
  views: SharedBoidViews
): Uint32Array {
  const activeIndex = Atomics.load(views.bufferIndex, 0)
  return activeIndex === 0
    ? views.stanceEnteredAtFrame0
    : views.stanceEnteredAtFrame1
}

/**
 * Get the currently inactive stanceEnteredAtFrame buffer (for worker to write to) - Session 130
 */
export function getInactiveStanceEnteredAtFrame(
  views: SharedBoidViews
): Uint32Array {
  const activeIndex = Atomics.load(views.bufferIndex, 0)
  return activeIndex === 0
...
/**
 * Pack stance and seekingMate into a single byte
 *
 * Layout:
...
/**
 * Unpack stance and seekingMate from a single byte
 *
 * @param flags - Packed byte value
 * @returns Object with stance and seekingMate
 */
export function unpackStanceFlags(flags: number): {
  stance: number
  seekingMate: boolean
} {
  return {
