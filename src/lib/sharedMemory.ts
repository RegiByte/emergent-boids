...
export function getSharedArrayBufferStatus(): {
  ...
  if (!isCrossOriginIsolated) {
    return {
      supported: false,
      reason:
        "crossOriginIsolated is false - needs COOP/COEP headers",
      crossOriginIsolated: isCrossOriginIsolated,
    };
  }
 ...
 */
export interface SharedBoidBufferLayout {
 ...
 */
export function calculateBufferLayout(boidCount: number): SharedBoidBufferLayout {
  let offset = 0;
 ...
 */
export interface SharedBoidViews {
...
export function swapBuffers(views: SharedBoidViews): void {
  const currentIndex = Atomics.load(views.bufferIndex, 0);
  const newIndex = currentIndex === 0 ? 1 : 0;
  Atomics.store(views.bufferIndex, 0, newIndex);
