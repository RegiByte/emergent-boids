...
export function lloydRelaxation(
  ...
  // Sample the space to find cell centroids
  ...
  // Average to get centroids, constrain to radius
...
export type VoronoiRenderOptions = {
  ...
  // Elliptical shape control (Session 103)
  ellipseRatio?: number; // Width/height ratio (1.0 = circle, >1 = wider, <1 = taller)

  // 3-channel marker control (Session 103)
  // Allows separate RGB channels for different shell regions
  ...
  // edgeColor is used for scute lines (e.g., BLUE channel)
};
...
export function drawVoronoi(
  ...
  // Scale seeds to match ellipse (for proper Voronoi calculation)
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      // Parse color and write to ImageData
  ...
  // putImageData ignores transforms, so we need to get the current transform
  // and apply it manually to position correctly
